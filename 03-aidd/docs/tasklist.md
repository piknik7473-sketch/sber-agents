...
| 1 - Базовый бот | ⏳ В процессе | Эхо-бот для проверки работы Telegram API |
| 2 - Интеграция с LLM | ⏸️ Ожидает | Подключение OpenRouter и вызов модели |
...
- [ ] Создать структуру `bot.py` (импорты, конфигурация, инициализация)
- [ ] Добавить хендлер на текст сообщения
- [ ] Реализовать эхо-ответ (бот возвращает сообщение пользователя)
- [ ] Добавить логирование через `print()` (старт, получение сообщения)
- [ ] Протестировать: отправить сообщение в бот, получить эхо-ответ
