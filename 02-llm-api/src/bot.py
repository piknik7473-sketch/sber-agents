...
class ChatBot:
    ...
    def _reset_history_with_system_prompt(self):
        """Сбросить историю и добавить системный промпт."""
    ...
    def summarize_history(self):
        """Суммаризовать длинную историю диалога (стратегия 3: скользящее окно с приоритетом)."""
        console.print("[yellow]📝 История слишком длинная, применяю скользящее окно...[/yellow]")
        
        # Извлекаем системный промпт и все остальные сообщения
        system_message = None
        other_messages = []
        ...
        if len(other_messages) <= (MAX_MESSAGES - 1):
            # Если сообщений немного (учитывая системный промпт), ничего не делаем
            return
        
        # Стратегия 3: сохраняем первые 2-3 сообщения + последние N сообщений
        KEEP_FIRST = 2  # Количество первых сообщений для сохранения (2-3 согласно требованиям)
        # Максимальное количество сообщений для пользователя и ассистента (MAX_MESSAGES - системный промпт)
        MAX_USER_MESSAGES = MAX_MESSAGES - 1
        
        # Определяем, что сохраняем
        first_messages = other_messages[:KEEP_FIRST]
        remaining_slots = MAX_USER_MESSAGES - KEEP_FIRST
        last_messages = other_messages[-remaining_slots:] if len(other_messages) > KEEP_FIRST + remaining_slots else other_messages[KEEP_FIRST:]
        
        # Собираем новую историю
        self.conversation_history = []
        if system_message:
            self.conversation_history.append(system_message)
        
        # Добавляем первые сообщения
        self.conversation_history.extend(first_messages)
        
        # Добавляем последние сообщения
        self.conversation_history.extend(last_messages)
        
        console.print(f"[green]✓ История оптимизирована: сохранено {len(self.conversation_history)} сообщений[/green]\n")
